...
from roles_api_mysql.settings import Settings
...
def config():
    with patch('roles_api_mysql.settings._conf.config', dummy_settings()):
        yield
...
@pytest.fixture(scope='session')
@patch("roles_api_mysql.settings.build_config", scope="session")
def manager(loop, mock_config):
    mock_config.return_value = None
    from roles_api_mysql.db import get_manager, pool
...
def cli(mock_config, loop, aiohttp_client):
    app = web.Application()
    from roles_api_mysql.app import make_app
